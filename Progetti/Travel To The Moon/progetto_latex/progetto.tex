...
Specifica delle chiavi primarie della classe Crociera:
\begin{enumerate}
    \item codice
\end{enumerate}

\hyperref[sec:VincoliCrociera]{Vincoli della classe Crociera}
...
\hyperref[sec:VincoliNave]{Vincoli della classe Nave}
...
Specifica delle chiavi primarie della classe Destinazione:
\begin{enumerate}
    \item $($nome, dest\_cit$)$
\end{enumerate}

\hyperref[sec:VincoliDestinazione]{Vincoli della classe Destinazione}
...
Specifica delle chiavi primarie della classe Prenotazione:
\begin{enumerate}
    \item $($pren\_croc, istante\_prenotazione$)$
\end{enumerate}

\hyperref[sec:VincoliPrenotazione]{Vincoli della classe Prenotazione}

\newpage
\subsection*{Classe\ Pren\_accettata}\label{sec:PrenotazioneAccettata}

\newpage
\subsection*{Classe\ Pren\_rifiutata}\label{sec:PrenotazioneRifiutata}
...
Specifica delle chiavi primarie della classe Città:
\begin{enumerate}
    \item $($nome, cit\_naz$)$
\end{enumerate}

\hyperref[sec:VincoliCittà]{Vincoli della classe Città}
...
Specifica delle chiavi primarie della classe Nazione:
\begin{enumerate}
    \item nome
\end{enumerate}

\hyperref[sec:VincoliNazione]{Vincoli della classe Nazione}
...
\subsection*{\label{sec:OperazioneGetPostiDisponibili}[getPostiDisponibili$($Crociera c$)$]}
\begin{enumerate}
    \item Restituisce il numero di posti disponibili data una determinata Crociera.
    \item precondizioni: $\exists$ n $|$ nave\_croc$($n,c$)$
    \item postcondizioni: no side effect
    \item P = getPrenotazioniAccettate$($Crociera c $)$
    \item num\_posti = num\_posti\_disponibili$($n$)$ $-$ $\sum_{p \in P} num\_posti\_prenotati(p)$
    \item return num\_posti
\end{enumerate}

\subsection*{\label{sec:OperazioneGetPrenotazioniAccettate}[getPrenotazioniAccettate$($Crociera c$)$]}
\begin{enumerate}
    \item Restituisce l'insieme delle prenotazioni accettate di una determinata Crociera.
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item return $\{p \in Prenotazione: pren\_croc(p,c) \land Pren\_accettata(p)\}$
\end{enumerate}

\subsection*{\label{sec:OperazioneEffettuaRichiestaPrenotazione}[effettuaRichiestaPrenotazione$($Crociera c, Utente u, InteroGZ num\_posti$)$]}
\begin{enumerate}
    \item Permette all'Utente di effettuare una richiesta di prenotazione. Restituisce un oggetto di tipo Prenotazione.
    \item precondizioni: nessuna
    \item postcondizioni: viene modificato l'aspetto estensionale dei dati
    \item Viene creato una nuova istanza p della classe Prenotazione
    \item istante\_prenotazione$($p$)$ = istante\_corrente
    \item num\_posti\_prenotati$($p$)$ = num\_posti
    \item Viene creata l'associazione pren\_croc$(p,c)$
    \item Viene creata l'associazione pren\_utente$(p,u)$
    \item return p
\end{enumerate}

\subsection*{\label{sec:OperazioneIsDestinazioneEsotica}[isDestinazioneEsotica$($Destinazione d$)$]}
\begin{enumerate}
    \item Restituisce vero se la destinazione è esotica (se il suo continente è diverso da Europa), altrimenti falso.
    \item precondizioni: $\exists$ cit, naz, cont $|$ dest\_cit$($d,cit$)$ $\land$ cit\_naz$($cit,naz$)$ $\land$ naz\_cont$($naz,cont$)$
    \item postcondizioni: no side effect
    \item return: nome$($cont$)$ $\neq$ Europa
\end{enumerate}

\subsection*{\label{sec:OperazioneIsPrenotazioneInPeriodo}[isPrenotazioneInPeriodo$($Prenotazione p, Data inizio, Data fine$)$]}
\begin{enumerate}
    \item Restituisce vero se la prenotazione è in un periodo compreso tra inizio e fine, altrimenti falso.
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item return istante\_prenotazione$($p$)$ $\geq$ inizio $\land$ istante\_prenotazione$($p$)$ $\leq$ fine
\end{enumerate}

\subsection*{\label{sec:OperazioneGetEtàMediaUtenti}[getEtàMediaUtenti$($Utente[] U$)$]}
\begin{enumerate}
    \item Restituisce l'età media degli utenti.
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item return $\frac{\sum_{i=0}^{|U|} getEta(U[i])}{|U|}$
    \item (operazione \hyperref[sec:OperazioneGetEta]{getEta})
\end{enumerate}

\subsection*{\label{sec:OperazioneGetEta}[getEta$($Utente u$)$]}
\begin{enumerate}
    \item Restituisce l'età dell'utente.
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item return istante\_corrente $-$ data\_di\_nascita$($u$)$
\end{enumerate}

\subsection*{\label{sec:OperazioneGetPrenotazioniInUnDatoPeriodo}[getPrenotazioniInUnDatoPeriodo$($Data inizio, Data fine$)$]}
\begin{enumerate}
    \item Restituisce l'insieme delle prenotazioni accettate in un dato periodo.
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item return $\{p \in Pren\_accettata: $\hyperref[sec:OperazioneIsPrenotazioneInPeriodo]{isPrenotazioneInPeriodo}$(p, inizio, fine)\}$
\end{enumerate}

\subsection*{\label{sec:OperazioneGetCrociereDaPrenotazioni}[getCrociereDaPrenotazioni$($Prenotazione[] P$)$]}
\begin{enumerate}
    \item Restituisce l'insieme delle crociere a partire da un insieme di prenotazioni.
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item return $\{c \in Crociera: p \in P \land pren\_croc(p,c)\}$
\end{enumerate}

\subsection*{\label{sec:OperazioneGetDestinazioniDaCrociere}[getDestinazioniDaCrociere$($Crociere[] C$)$]}
\begin{enumerate}
    \item Restituisce l'insieme delle destinazioni a partire da un insieme di crociere.
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item return $\{d \in Destinazione: \exists c \in C: tappa\_dest(t,d) \land t \in Tappa \land tappa\_itin(t,I) \land I \in Itinerario \land itin\_croc(I,c)\}$
\end{enumerate}
...
\subsection*{\label{sec:OperazioneGetCrocieraPerFamiglieCheRaggiungeUnaDataDestinazioneInUnDatoPeriodo}[getCrocieraPerFamiglieCheRaggiungeUnaDataDestinazioneInUnDatoPeriodo$($Destinazione d, Data inizio, Data fine$)$]}
\begin{enumerate}
    \item Restituisce le crociere per famiglie che raggiungono una data destinazione in un dato periodo.
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item P = \hyperref[sec:OperazioneGetCrocierePerFamiglieInUnDatoPeriodo]{getCrocierePerFamiglieInUnDatoPeriodo}$($inizio, fine$)$
    \item D = \hyperref[sec:OperazioneGetCrocierePerFamiglieInDestinazione]{getCrocierePerFamiglieInDestinazione}$($d$)$
    \item return P $\cap$ D
\end{enumerate}

\subsection*{\label{sec:OperazioneGetCrocieraLunaDiMieleCheRaggiungeUnaDataDestinazioneInUnDatoPeriodo}[getCrocieraLunaDiMieleCheRaggiungeUnaDataDestinazioneInUnDatoPeriodo$($Destinazione d, Data inizio, Data fine$)$]}
\begin{enumerate}
    \item Restituisce le crociere luna di miele che raggiungono una data destinazione in un dato periodo.
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item P = \hyperref[sec:OperazioneGetCrociereLunaDiMieleInUnDatoPeriodo]{getCrociereLunaDiMieleInUnDatoPeriodo}$($inizio, fine$)$
    \item D = \hyperref[sec:OperazioneGetCrociereLunaDiMieleInDestinazione]{getCrociereLunaDiMieleInDestinazione}$($d$)$
    \item return P $\cap$ D
\end{enumerate}

\subsection*{\label{sec:OperazioneIsCrocieraInPeriodo}[isCrocieraInPeriodo$($Crociera c, Data inizio, Data fine$)$]}
\begin{enumerate}
...
\subsection*{\label{sec:OperazioneGetCrocierePerFamiglieInUnDatoPeriodo}[getCrocierePerFamiglieInUnDatoPeriodo$($Data inizio, Data fine$)$]}
\begin{enumerate}
    \item Restituisce le crociere per famiglie in un dato periodo.
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item return $\{c \in Crociere: $\hyperref[sec:OperazioneIsCrocieraInPeriodo]{isCrocieraInPeriodo}$(c, inizio, fine) \land c \in CrocierePerFamiglie\}$
\end{enumerate}

\subsection*{\label{sec:OperazioneGetCrociereLunaDiMieleInUnDatoPeriodo}[getCrociereLunaDiMieleInUnDatoPeriodo$($Data inizio, Data fine$)$]}
\begin{enumerate}
    \item Restituisce le crociere luna di miele in un dato periodo.
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item return $\{c \in Crociere: $\hyperref[sec:OperazioneIsCrocieraInPeriodo]{isCrocieraInPeriodo}$(c, inizio, fine) \land c \in CrociereLunaDiMiele\}$
\end{enumerate}

\subsection*{\label{sec:OperazioneGetCrociereLunaDiMieleInDestinazione}[getCrociereLunaDiMieleInDestinazione$($Destinazione d$)$]}
\begin{enumerate}
    \item Restituisce le crociere luna di miele che raggiungono una data destinazione.
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item return $\{c \in CrociereLunaDiMiele: $\hyperref[sec:OperazioneIsCrocieraInDestinazione]{isCrocieraInDestinazione}$(c, d)\}$
\end{enumerate}

\subsection*{\label{sec:OperazioneGetCrocierePerFamiglieInDestinazione}[getCrocierePerFamiglieInDestinazione$($Destinazione d$)$]}
\begin{enumerate}
    \item Restituisce le crociere per famiglie che raggiungono una data destinazione.
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item return $\{c \in CrocierePerFamiglie: $\hyperref[sec:OperazioneIsCrocieraInDestinazione]{isCrocieraInDestinazione}$(c, d)\}$
\end{enumerate}

\subsection*{\label{sec:OperazioneIsCrocieraInDestinazione}[isCrocieraInDestinazione$($Crociera c, Destinazione d$)$]}
\begin{enumerate}
    ...
    \item return 
    \begin{enumerate}
        \item true se esistono i, t tale che tappa\_itin$(t, i)$ $\land$ tappa\_dest$(t, d)$ $\land$ itin\_croc$(i, c)$
