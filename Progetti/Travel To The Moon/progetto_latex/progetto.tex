...
\begin{enumerate}
    ...
    \item P = getPrenotazioni$($Crociera c $)$
    \item num\_posti = num\_posti\_disponibili$($n$)$ $-$ $\sum_{p \in P} num\_posti\_prenotati(p)$
    \item return num\_posti
\end{enumerate}

\subsection*{[getPrenotazioni$($Crociera c$)$]}
\begin{enumerate}
    \item Restituisce l'insieme delle prenotazioni di una determinata Crociera.
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item return $\{p \in Prenotazione: pren\_croc(p,c)\}$
\end{enumerate}
...
\begin{enumerate}
    ...
    \item precondizioni: getPostiDisponibili$(c)$ $\geq$ num\_posti $\land$ istante\_corrente $\leq$ data\_inizio$(c)$
    \item postcondizioni:
    \item Viene creata una nuova prenotazione p:
    \begin{enumerate}
...
\begin{enumerate}
    ...
    \item postcondizioni: no side effect
    \item return: naz\_cont$($naz,cont$)$ $\neq$ nome$($cont$)$ $\neq$ Europa
\end{enumerate}

\subsection*{[isAlmenoUnaDestinazioneEsotica$($Destinazione[] D$)$]}
\begin{enumerate}
    \item Restituisce vero se almeno una destinazione è esotica, altrimenti falso.
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item return true se $\exists d \in D: isDestinazioneEsotica(d)$
...
\begin{enumerate}
    ...
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item return $\frac{\sum_{i=0}^{|U|} get\_eta(U[i])}{|U|}$
\end{enumerate}

\subsection*{[get\_eta$($Utente u$)$]}
\begin{enumerate}
...
\subsection*{[getPrenotazioniInUnDatoPeriodo$($Data inizio, Data fine$)$]}
\begin{enumerate}
    \item Restituisce l'insieme delle prenotazioni in un dato periodo.
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item return $\{p \in Prenotazione: isPrenotazioneInPeriodo(p, inizio, fine)\}$
\end{enumerate}

\subsection*{[getCrociereDaPrenotazioni$($Prenotazione[] P$)$]}
\begin{enumerate}
...
\subsection*{[getDestinazioniDaCrociere$($Crociere[] C$)$]}
\begin{enumerate}
...
\begin{enumerate}
    ...
    \item precondizioni: nessuna
    \item postcondizioni: no side effect
    \item P = getPrenotazioniInUnDatoPeriodo$($inizio, fine$)$
    \item C = getCrociereDaPrenotazioni$($P$)$
    \item D = getDestinazioniDaCrociere$($C$)$
    \item D\_esotiche = $\{d \in D: isDestinazioneEsotica(d)\}$
    ...
    \item return getEtàMediaUtenti$($U$)$
\end{enumerate}
